...
{
    uint8_t rxBuff[ 1 ];
    bmp581_read( BMP581_PRESS_DATA_LSB, rxBuff, 1, &fakeBmp );
...
TEST( BMP581, ReadReceivesOnlySpecifiedNumOfBytes )
{
    ...
    uint8_t rxBuff[ 2 ];
    bmp581_read( BMP581_PRESS_DATA_LSB , rxBuff, sizeof( rxBuff ), &fakeBmp );
...
{
    uint8_t data = 0x01;
    bmp581_write( BMP581_FIFO_CONFIG, data, &fakeBmp ); 
