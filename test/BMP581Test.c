...
TEST( BMP581, RequireFullInterface )
{
    BMP581Interface inter =
    {
        .i2cWrite = WriteSpy_Write8Arr,
    ...
    TEST_ASSERT_EQUAL( BMP581_FAIL, BMP581_SetInterface( &inter ) );

    inter.i2cRead = FakeRead_Read8ArrDiffSize;
    TEST_ASSERT_EQUAL( BMP581_SUCCESS, BMP581_SetInterface( &inter ) );
}
...
{
    uint8_t rxBuff[ 1 ];
    BMP581_Read( BMP581_SLAVE_ADDRESS, BMP581_PRESS_DATA_LSB, rxBuff, 1 );
...
TEST( BMP581, ReadReceivesOnlySpecifiedNumOfBytes )
{
    ...
    uint8_t rxBuff[ 2 ];
    BMP581_Read( BMP581_SLAVE_ADDRESS, BMP581_PRESS_DATA_LSB , rxBuff, sizeof( rxBuff ) );
...
{
    uint8_t data = 0x01;
    BMP581_Write( BMP581_SLAVE_ADDRESS, BMP581_FIFO_CONFIG, data ); 
